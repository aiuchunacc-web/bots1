...
dp = Dispatcher()
# Gemini client. Gemini API has a free tier for supported models, but it still
# has rate limits. We batch questions to keep request count low.
GEMINI_TIMEOUT = max(15.0, float(os.getenv("GEMINI_TIMEOUT", "90")))
GEMINI_MAX_RETRIES = max(0, min(5, int(os.getenv("GEMINI_MAX_RETRIES", "3"))))
GEMINI_MAX_RETRY_WAIT = max(5.0, float(os.getenv("GEMINI_MAX_RETRY_WAIT", "60")))
ai = genai.Client(api_key=GEMINI_API_KEY, http_options=types.HttpOptions(timeout=int(GEMINI_TIMEOUT * 1000)))
...
# Set GEMINI_BATCH_SIZE in Render if desired. 40 is a good balance for normal MCQs.
GEMINI_BATCH_SIZE = max(1, min(50, int(os.getenv("GEMINI_BATCH_SIZE", "40"))))
GEMINI_MIN_REQUEST_INTERVAL = max(0.0, float(os.getenv("GEMINI_MIN_REQUEST_INTERVAL", "1.3")))
ai_limiter = AIRequestLimiter(GEMINI_MIN_REQUEST_INTERVAL)


def retry_after_seconds(exc: Exception) -> float | None:
    """Extract a useful retry delay from Gemini error text when present."""
    text = str(exc)
...
""" + "\n\n---\n\n".join(blocks)
    ...
    response = await ai.aio.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            temperature=0,
            max_output_tokens=max(32, len(items) * 4),
            candidate_count=1,
        ),
    )
...
async def solve_questions(
    ...
    - The Gemini SDK's hidden retries are disabled, so we control request count.
    - Short temporary 429/5xx/timeouts are retried a few times.
    - If Gemini asks us to wait longer than GEMINI_MAX_RETRY_WAIT, we STOP instead
    ...
    for start in range(0, total, GEMINI_BATCH_SIZE):
        ...
        for attempt in range(GEMINI_MAX_RETRIES + 1):
            try:
                ...
                if delay > GEMINI_MAX_RETRY_WAIT or attempt >= GEMINI_MAX_RETRIES:
                    raise RuntimeError(
                        "Gemini hozircha so‘rovni qabul qilmayapti. "
    ...
    if flow.get("step") == "title":
        ...
        try:
            ...
            await message.answer(
                ...
                "Agar Gemini limiti bo‘lsa, bot endi cheksiz kutib qolmaydi. "
